...
class VideoFaceAnalyzer:
    ...
    def _classify_head_pose(self) -> str:
        yaw_flag = abs(self.pose_angles.yaw) > self.yaw_threshold
        pitch_flag = abs(self.pose_angles.pitch) > self.pitch_threshold
        roll_flag = abs(self.pose_angles.roll) > self.roll_threshold

        if yaw_flag:
            return "LOOKING AWAY"
        if pitch_flag:
            return "LOOKING UP/DOWN"
        if roll_flag:
            return "HEAD TILT"
        return "FORWARD"
    ...
    def draw_mediapipe(self, frame):
        ...
        if results.multi_face_landmarks:
            ...
            pose_label = self._classify_head_pose()
            cv2.putText(
                frame,
                (
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=float,
        default=25.0,
        help="좌우 회전(Yaw) 이상 판정 임계값(도)",
    )
    parser.add_argument(
        "--pitch-threshold",
        type=float,
        default=20.0,
        help="위아래 회전(Pitch) 이상 판정 임계값(도)",
    )
    parser.add_argument(
        "--roll-threshold",
        type=float,
        default=15.0,
        help="고개 기울기(Roll) 이상 판정 임계값(도)",
    )
    return parser
